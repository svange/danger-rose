...
class AudioValidator:
                ...
                try:
                    ...
                    if not pygame.mixer.music.get_busy():
                        results["playback_errors"].append({
                            "scene": scene,
                            "issue": "Music failed to start playing"
                        })
                        self.issues.append(AudioIssue(
            ...
            if sfx_path.exists():
                try:
                    ...
                    if not channel:
                        results["playback_errors"].append({
                            "sfx": sfx_name,
                            "issue": "Failed to get playback channel"
                        })
    ...
    def _test_audio_integration(self) -> Dict[str, any]:
        ...
        try:
            ...
            results["integration_tests"].append({
                "test": "import_sound_manager",
                "issue": f"Failed to import: {str(e)}"
            })
    ...
    def _generate_report(self, test_results: Dict[str, any]) -> Dict[str, any]:
        ...
        # Save report
        report_path = self.results_dir / f"audio_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
