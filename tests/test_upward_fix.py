...
def test_upward_fix():
    ...
    for character_name, sprite_path in [("danger", get_danger_sprite()), ("rose", get_rose_sprite())]:
        ...
        for method, y_start in [("original", original_y), ("wrong_down", wrong_y), ("correct_up", correct_y)]:
            ...
            viz = sheet.copy()
            color = (255, 0, 0) if method == "wrong_down" else (0, 255, 0) if method == "correct_up" else (0, 0, 255)
            pygame.draw.rect(viz, color, (0, y_start, sheet.get_width(), frame_height), 4)
        ...
        print(f"    Saved {attack_char.get_frame_count()} frames using AttackCharacter class")
