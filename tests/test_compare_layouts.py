...
def test_layout_comparison():
    ...
    for character_name, sprite_path in [("danger", get_danger_sprite())]:  # Just test one for now
        ...
        print(f"Old layout (4x4): {old_frame_width}x{old_frame_height}, attack row at y={old_y_start}")
        ...
        print(f"New layout (3x4): {new_frame_width}x{new_frame_height}, attack row at y={new_y_start}")
        ...
        for layout_name, frame_width, frame_height, y_start in [
            ...
            viz = sheet.copy()
            pygame.draw.rect(viz, (255, 0, 0), (0, y_start, sheet.get_width(), frame_height), 4)
