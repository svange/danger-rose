...
class VisualRegressionTester:
    ...
    def __init__(self, reference_dir: str = "visual_references"):
        ...
        self.current_test_dir = self.results_dir / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def run_visual_tests(self) -> Dict[str, any]:
        ...
        for test_name, test_func in test_scenarios:
            ...
            try:
                ...
                for screenshot_name, screenshot_path in test_results.get("screenshots", {}).items():
                    self._analyze_screenshot_for_bugs(screenshot_path, test_name, screenshot_name)
    ...
    def _compare_with_reference(self, current_path: str, test_name: str) -> Optional[VisualRegression]:
        ...
        if diff_percentage > 1.0:  # 1% threshold
            ...
            pixel_diff_count = np.sum(diff_array > 10)  # Pixels with significant difference
    ...
    def _analyze_screenshot_for_bugs(self, screenshot_path: str, test_name: str, 
    ...
    def _detect_color_corruption(self, img_array: np.ndarray, name: str) -> Optional[str]:
    ...
    def _detect_ui_misalignment(self, img_array: np.ndarray, name: str) -> Optional[str]:
    ...
    def _detect_animation_glitch(self, img_array: np.ndarray, name: str) -> Optional[str]:
    ...
    def _check_sprite_quality(self, screen: pygame.Surface, character: str, 
                            animation: str) -> List[str]:
        ...
        if gray_count < total_pixels * 0.5:  # Less than 50% background visible
            issues.append(f"{character} {animation}: Sprite may be too large or have transparency issues")
    ...
    def _detect_animation_glitches(self, frame_paths: List[str]) -> List[str]:
        ...
        for i in range(1, len(frames)):
            ...
            if avg_diff > 100:  # Threshold
                glitches.append(f"Large change between frames {i-1} and {i}: {avg_diff:.1f}")
    ...
    def _generate_report(self, results: Dict[str, any]) -> Dict[str, any]:
        ...
        report = {
            ...
            "total_screenshots": sum(len(r.get("screenshots", {})) for r in results.values()),
    ...
    def _print_summary(self, report: Dict[str, any]) -> None:
        ...
        if report['visual_regressions'] > 0:
            ...
            for regression in report['regressions']:
                print(f"  - {regression['test']}: {regression['difference']} difference")
            ...
            if isinstance(test_result, dict):
                for key in ['issues', 'alignment_issues', 'animation_issues']:
                    if key in test_result:
                        issue_counts[key] = issue_counts.get(key, 0) + len(test_result[key])
