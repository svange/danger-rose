...
class TestSpriteCutting:
    ...
    def test_sprite_sheet_dimensions(self):
        ...
        if os.path.exists(danger_path):
            ...
            print(f"Danger sheet dimensions: {danger_sheet.get_width()}x{danger_sheet.get_height()}")
            
        if os.path.exists(rose_path):
            rose_sheet = pygame.image.load(rose_path)
            print(f"Rose sheet dimensions: {rose_sheet.get_width()}x{rose_sheet.get_height()}")
    ...
    def test_sprite_cutting_visualization(self):
        ...
        for character_name, sprite_path in [("danger", danger_path), ("rose", rose_path)]:
            ...
            pygame.image.save(original_sheet, str(output_dir / f"{character_name}_original.png"))
            ...
            frame_sizes = [
                ...
                (32, 32),  # Smaller
                (48, 48),  # Medium
                (sheet_width // 4, sheet_height // 3),  # Calculated based on 4 cols, 3 rows
                (sheet_width // 3, sheet_height // 3),  # Calculated based on 3 cols, 3 rows
            ]
            ...
            for frame_width, frame_height in frame_sizes:
                ...
                frame_dir = output_dir / f"{character_name}_{frame_width}x{frame_height}"
                ...
                for row in range(3):  # 3 rows
                    for col in range(4):  # Assume 4 columns max
                        ...
                        frame.blit(original_sheet, (0, 0), (x, y, frame_width, frame_height))
                ...
                print(f"Generated {frame_count} frames for {frame_width}x{frame_height}")
    
    def test_current_animation_loading(self):
        """Test the current animation loading system."""
        print(f"\n=== CURRENT ANIMATION LOADING TEST ===")
        
        for character_name, sprite_path in [("danger", get_danger_sprite()), ("rose", get_rose_sprite())]:
            if not os.path.exists(sprite_path):
                print(f"Skipping {character_name} - file not found")
                continue
            ...
            output_dir = Path(__file__).parent.parent / "test-artifacts" / "sprite_output" / f"{character_name}_current_system"
    ...
    def test_calculate_optimal_frame_size(self):
        ...
        for character_name, sprite_path in [("danger", get_danger_sprite()), ("rose", get_rose_sprite())]:
            ...
            grid_options = [
                (4, 3, "4 cols, 3 rows"),  # 4 walking, 4 jumping, 3 attacking + 1 effect
...
if __name__ == "__main__":
    ...
    print(f"\n=== TEST COMPLETE ===")
    print(f"Check the 'tests/sprite_output' directory for visual results!")