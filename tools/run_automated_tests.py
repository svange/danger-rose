...
class AutomatedTestRunner:
            ...
            try:
                ...
                if result.get("status") == "passed":
                    ...
                    if test.get("critical"):
                        print("⚠️  Critical test failed - subsequent tests may be affected")
    ...
    def _run_test(self, test_config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            output_dir = self.session_dir / test_config["name"].lower().replace(" ", "_")
    ...
    def _determine_test_status(self, test_name: str, result: Dict) -> str:
        ...
        if test_name == "Audio Validation":
            ...
            if result.get("crashes", 0) > 0:
                return "failed"
            elif result.get("visual_bugs", 0) > 5 or result.get("performance_issues", 0) > 10:
                return "warning"
            return "passed"
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if audio_result.get("status") == "failed":
            ...
            if details.get("test_results", {}).get("file_validation", {}).get("missing"):
                recommendations.append("🔊 Add missing audio files to prevent runtime errors")
            if details.get("critical_issues", 0) > 0:
                recommendations.append("🔊 Fix critical audio issues (corrupted files, playback failures)")
        
        # Check visual issues
        visual_result = self.test_results.get("Visual Regression", {})
        if visual_result.get("status") == "failed":
            recommendations.append("🎨 Review visual regressions and update reference images if changes are intentional")
        ...
        if game_result.get("status") in ["failed", "warning"]:
            ...
            if details.get("crashes", 0) > 0:
                recommendations.append("🐛 Fix game crashes immediately - check crash reports for details")
            if details.get("visual_bugs", 0) > 5:
                recommendations.append("🎮 Investigate visual bugs (missing sprites, black screens)")
    ...
    def _generate_test_results_html(self, test_results: Dict) -> str:
        ...
        for test_name, result in test_results.items():
            ...
            if status == "crashed":
                ...
                if test_name == "Audio Validation":
                    file_val = details.get("test_results", {}).get("file_validation", {})
                    html += f"""
                    <table>
...
def main():
    ...
    parser.add_argument("--quick", action="store_true", help="Run quick tests (30s instead of 2min)")
    parser.add_argument("--visual-only", action="store_true", help="Run only visual tests")
